defmodule EshopWeb.UserPermissionController do
  ...
  def index(conn, _params) do
    user_permissions = Accounts.list_user_permissions()
    render(conn, "index.json", user_permissions: user_permissions)
  end

  def create(conn, %{"user_permission" => user_permission_params}) do
    with {:ok, %UserPermission{} = user_permission} <- Accounts.create_user_permission(user_permission_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    user_permission = Accounts.get_user_permission!(id)
  ...
  def update(conn, %{"id" => id, "user_permission" => user_permission_params}) do
    ...
    with {:ok, %UserPermission{} = user_permission} <- Accounts.update_user_permission(user_permission, user_permission_params) do
      render(conn, "show.json", user_permission: user_permission)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %UserPermission{}} <- Accounts.delete_user_permission(user_permission) do
