defmodule EshopWeb.Schema.Resolvers.ItemWarehouseVariant do
  ...
  def update_item_warehouse_variant(
        %{id: id, item_warehouse_variant_params: item_warehouse_variant_params},
        _info
      ) do
    case {:ok, Eshop.Components.get_item_warehouse_variant!(id)} do
      {:ok, item_warehouse_variant} ->
        item_warehouse_variant
        |> Eshop.Components.update_item_warehouse_variant(item_warehouse_variant_params)
    end
  end
  ...
  def delete_item_warehouse_variant(%{id: id}, _info) do
    case {:ok, Eshop.Components.get_item_warehouse_variant!(id)} do
      {:ok, item_warehouse_variant} ->
        item_warehouse_variant |> Eshop.Components.delete_item_warehouse_variant()
    end
  end
end
